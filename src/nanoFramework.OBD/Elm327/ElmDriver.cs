...
namespace Elm327.Core {
	...
	public class ElmDriver : IDisposable {
		...
		/// <summary>
		/// Fired when the ELM reports that it is unable to connect to the vehicle via one of the OBD protocols. Note that although the event is named <see cref="ObdConnectionLost" />, it could also
		...
		/// <summary>Possible results of calling the <see cref="Elm327.Core.ElmDriver.Connect" />() method.</summary>
		...
		#endregion Structures/Enumerations
		...
		/// <param name="protocolType">The desired OBD protocol to use. Using <see cref="Elm327.Core.ElmDriver.ElmObdProtocolType.Automatic" /> is usually a good idea.</param>
		...
		public double BatteryVoltage {
			get {
				try {
					// The reading will come back as something like "12.7V", so get rid of the 'V' and Double.Parse() the rest
		...
		/// <summary>
		/// Gets or sets the current OBD protocol. Note that if this value was originally set to <see cref="Elm327.Core.ElmDriver.ElmObdProtocolType.Automatic" />, the value you get when requesting
		...
		private string ReceiveMessage() {
			try {
				lock (this.serialPort) {
					// Keep reading from the serial connection until the ELM sends the ready prompt character or we time out. We wait for the ready prompt because while most responses from the ELM
					// will be only one line, some will be multiline, so we need to keep reading lines until we receive the ready prompt.
						...
						if (this.serialPort.BytesToRead > 0 && this.serialPort.Read(this.receiveBuffer, i, 1) > 0) {
							if (this.receiveBuffer[i] == ElmDriver.CHIP_READY_PROMPT_CHAR) {
								...
								if (message.IndexOf(ElmDriver.MESSAGE_NO_DATA) > -1) {
									// A request timed out or a PID not supported by an ECU was requested

									return null;
								}
								else if (message.IndexOf(ElmDriver.MESSAGE_NO_CONNECTION) > -1) {
									// The ELM is unable to connect to the vehicle's OBD system for some reason
									...
									// If the ELM returned a "SEARCHING..." message, trim it out before returning to the caller
					...
					while (timeStarted.AddSeconds(ElmDriver.MAX_WAIT_RECEIVE_SECONDS) > DateTime.UtcNow);
		...
		public ElmConnectionResultType Connect() {
			// Try to force the connection closed if it is open and the caller wants to reinitialize
			...
			// Perform a reset on the ELM. If we get a null back from this call, it means we're unable to see it (probably a wiring issue).
			...
			// TODO: ELM responses will be faster if we turn off space printing, but leaving as-is for now to enable easy message parsing
			...
			// Set the caller's desired protocol, then make a simple "0100" call to make sure the ECU responds. If we get anything back other than something that starts with "41 00", it means the ELM
			// can't talk to the OBD system.
			...
			// Ask the ELM to give us the protocol it's using. We need to ask for this value in case the user chose the "Automatic" setting for protocol type, so we'll know which protocol was actually
			// selected by the ELM.
			...
			if (response != null && response.Length > 0) {
				try {
					// 'A' will be the first character returned if the user chose automatic search mode
