...
{
  internal class ReturnAst : StatementAst
  {
    public ExpressionAst expression;

    public ReturnAst(Token token)
      : base(token)
    {
      this.NodeType = AstNodeType.Return;
    }

    public void SetReturnExpression(ExpressionAst expression) => this.expression = expression;
  }
}
