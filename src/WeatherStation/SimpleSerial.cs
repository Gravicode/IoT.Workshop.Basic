...
namespace WeatherStation
{
    ...
    public class SimpleSerial
    {
        ...
        {
            port.Write(Encoding.UTF8.GetBytes(txt), 0, txt.Length);
        }
        ...
        {
            int arraySize = port.BytesToRead;

            byte[] received = new byte[arraySize];

            port.Read(received, 0, arraySize);
        ...
        {
            string receivedData = string.Concat(this._Remainder, port.ReadExisting());   // attach the previous remainder to the new data
