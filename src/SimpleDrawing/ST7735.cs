...
namespace SimpleDrawing
{
    public class ST7735 : IDisposable
    {
        ...
        public ST7735(int chipSelect, int dc, int reset, SpiDevice spiModule)
        {

            AutoRefreshScreen = false;
            var gpio = new GpioController();
            DataCommand = gpio.OpenPin(dc, PinMode.Output);
            DataCommand.Write(PinValue.Low);
            //new OutputPort(dc, false);
            Reset = gpio.OpenPin(reset,PinMode.Output);
            Reset.Write(PinValue.High);
            // new OutputPort(reset, true);
        ...
        protected GpioPin DataCommand;
        protected GpioPin Reset;
