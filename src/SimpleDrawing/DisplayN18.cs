...
namespace SimpleDrawing
{
    ...
    */
    public class DisplayN18
    {
        ...
        public DisplayN18(int resetPin, int backlightPin, int controlPin, int spiLine, int chipSelectPin)
        {
            _buffer1 = new byte[1];
            _buffer2 = new byte[2];
            _buffer4 = new byte[4];
            ...
            _controlPin = gpio.OpenPin(controlPin);
            _controlPin.SetPinMode(PinMode.Output);

            _resetPin = gpio.OpenPin(resetPin);
            _resetPin.SetPinMode(PinMode.Output);

            _backlightPin = gpio.OpenPin(backlightPin);
            _backlightPin.SetPinMode(PinMode.Output);
            _backlightPin.Write(PinValue.Low);

            //_chipSelectLine = gpio.OpenPin(chipSelectPin);
            //_chipSelectLine.SetPinMode(PinMode.Output);


            //var spiSettings = new SpiConnectionSettings(1);//_chipSelectLine.PinNumber
            //spiSettings.ClockFrequency = 12000000;
            //spiSettings.DataBitLength = 8;
            //spiSettings.Mode = SpiMode.Mode0;
            var con = GetConnectionSettings(PinValue.Low,chipSelectPin);

            _spi = SpiDevice.Create(con);
        ...
        {
            _controlPin.Write(PinValue.High);
        ...
        public void TurnOn()
        {
            _backlightPin.Write(PinValue.High);
        }
